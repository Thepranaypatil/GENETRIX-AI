...
const styleDescriptions: Record<ThumbnailStyle,String>={
    "Bold & Graphic" : "High contrast,bold typography,striking visuals",
     "Minimalist" : "Clen,Simple,lots of white space",
     "Photorealistic":"Photo-based,natural looking",
     "Illustrated":"Hand-drawn,artistic,creative",
     "Tech/Futuristic":"Modern,Sleek,teck-inspired",
}
...
const styleIcons: Record<ThumbnailStyle,React.ReactNode>={
  ...
  return (
    <div className="relative space-y-3 dark">
        ...
        <button type="button" onClick={()=>setIsOpen(!isOpen)} className="flex w-full items-center justify-between rounded-b-md border px-4 py-3 text-left transition bg-white/8 border-white/10 text-zinc-200 hover:bg-white/12">
            <div className="space-y-1">
           <div className="felx items-center gap-2 font-medium">
            {styleIcons[value]}
            <span>{value}</span>
           </div>
        ...
        {isOpen && (
        <div className="absolute bottom-0 z-50 mt-1 w-full rounded-md border border-white/12 bg-balck/20 backdrop-blur-3xl shadow=lg">
            {thumbnailStyles.map((style)=>(
                <button key={style} type="button" onClick={()=>{onChange(style); setIsOpen(false);}} className="flex w-full item-start gap-3 px-4 py-3 text-left transition hover::bg-black/30"> <div className="mt-0.5">{styleIcons[style]}</div>
                <div>
                    <p className="font-medium">{style}</p>
                    <p className="text-xs test-zinc-400">{styleDescriptions[style]}</p>
