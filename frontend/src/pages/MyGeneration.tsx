...
import { ArrowBigRightIcon, DownloadIcon, Trash2Icon } from "lucide-react"
              ...
              return (
                <div key={thumb._id} onClick={() => navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition-shadow-xl break-inside-avoid">
                  ...
                  <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                    ...
                    {thumb.isGenerating && <div className="absolute inset-0 bg-balck/50 flex items-center justify-center text-sm font-medium text-white">Generating...</div>}
                  ...
                  <div onClick={(e) => e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap=1.5">
                    ...
                    <Trash2Icon onClick={() => handleDelete(thumb._id)} className="size-6 bg-black/50 p-1 rounded hover:bg-pink-600 transition-all" />
                    ...
                    <Link target="_balnk" to={`/preview?thumbnail_url=${thumb.image_url}&title=${thumb.title}`}>


                      <ArrowBigRightIcon className="size-6 bg-black/50 p-1 rounded hover:bg-pink-600 transition-all" />
                    </Link>
                  </div>
                </div>
              )
